...
const env={APP_ORIGIN:'https://harmonious.example',SUPABASE_URL:'https://project.supabase.co',SUPABASE_PUBLISHABLE_KEY:'publishable-test',SUPABASE_SECRET_KEY:'sb_secret_test',SIGNUP_MODE:'public',TURNSTILE_SITE_KEY:'site-test',TURNSTILE_SECRET_KEY:'secret-test'};
assert(accountConfiguration(env));
for(const changes of [{TURNSTILE_SECRET_KEY:''},{TURNSTILE_SITE_KEY:''},{SUPABASE_SECRET_KEY:''},{SIGNUP_MODE:'typo'},{SIGNUP_MODE:'invite'},{SIGNUP_MODE:''}])assert(!accountConfiguration({...env,...changes}));
...
const auth=new AccountAuth(env,async(url,options)=>{calls.push({url,body:JSON.parse(options.body||'{}')});if(url.includes('siteverify'))return Response.json({success:options.body.includes('valid-token')||options.body.includes('fresh-token'),hostname:'harmonious.example',action:'signup'});if(providerError)return Response.json(providerError,{status:providerError.status||400});return Response.json(url.endsWith('/verify')?{user,access_token:'access',refresh_token:'refresh'}:{});});
...
assert.equal(calls.length,2,'Token is verified before the email request');
